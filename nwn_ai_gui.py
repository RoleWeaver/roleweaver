...
class NWNAIApp:
    ...
    def _build_guardrails_usage_tab(self, frame):
        ...
        ttk.Combobox(
            toolbar,
            textvariable=self.usage_window_var,
            values=["session", "24h", "7d", "30d"],
            state="readonly",
            width=9,
        ...
        ttk.Button(toolbar, text="Save settings", command=self._save_guardrail_settings).pack(
            side="right"
        )

        self.guardrail_status_var = tk.StringVar(value="Checking Guardrails AI...")
        ttk.Label(frame, textvariable=self.guardrail_status_var, wraplength=760).pack(
            fill="x", pady=(7, 5)
        )
        self.usage_summary_var = tk.StringVar(value="No usage recorded.")
        ttk.Label(frame, textvariable=self.usage_summary_var, font=("Segoe UI", 10, "bold")).pack(
            fill="x", pady=(0, 5)
        )

        self.usage_canvas = tk.Canvas(
            frame, height=125, bg="#1e1f22", highlightthickness=0
        )
        ...
        self.guardrail_input_limit_var = tk.StringVar(
            value=str(self.settings.get("guardrail_input_max_characters", 50000))
        )
        self.guardrail_output_limit_var = tk.StringVar(
            value=str(self.settings.get("guardrail_output_max_characters", 10000))
        )
        self.usage_input_rate_var = tk.StringVar(
            value="" if self.settings.get("usage_input_cost_per_million") is None else str(self.settings["usage_input_cost_per_million"])
        )
        self.usage_output_rate_var = tk.StringVar(
            value="" if self.settings.get("usage_output_cost_per_million") is None else str(self.settings["usage_output_cost_per_million"])
        )
        fields = [
            ("Maximum input characters", self.guardrail_input_limit_var),
            ("Maximum output characters", self.guardrail_output_limit_var),
            ("Input USD / million tokens", self.usage_input_rate_var),
            ("Output USD / million tokens", self.usage_output_rate_var),
        ]
        ...
        ttk.Label(
            settings,
            text="Costs are estimates using rates you enter. Unknown pricing is never shown as zero. "
            "No prompts, replies, Tells, translations, or API keys are stored in usage records.",
            wraplength=700,
        ).grid(row=0, column=2, rowspan=4, sticky="nw", padx=(18, 0))
    ...
    def _save_guardrail_settings(self):
        try:
            ...
            output_limit = int(self.guardrail_output_limit_var.get())
            if not 1 <= input_limit <= 1000000 or not 1 <= output_limit <= 1000000:
                raise ValueError("Character limits must be between 1 and 1,000,000.")
            rates = []
            for variable in (self.usage_input_rate_var, self.usage_output_rate_var):
                ...
                if value is not None and not 0 <= value <= 10000:
                    raise ValueError("Token rates must be between 0 and 10,000 USD per million.")
                rates.append(value)
            self.settings.update(
                guardrail_input_max_characters=input_limit,
                guardrail_output_max_characters=output_limit,
                usage_input_cost_per_million=rates[0],
                usage_output_cost_per_million=rates[1],
            )
            core.save_settings(self.settings)
            messagebox.showinfo(
                "Guardrails",
                "Guardrail and usage settings saved. Guardrail limits apply on the next Start.",
            )
    ...
    def _refresh_guardrails_usage(self):
        try:
            ...
            if status.get("available"):
                self.guardrail_status_var.set(
                    f"Guardrails AI {status.get('version') or ''} is active. Backend: {status.get('backend')}."
                )
            else:
                self.guardrail_status_var.set(
                    "DEGRADED: " + status.get("error", "Guardrails AI is unavailable.")
                )
            store = self.bot.usage_store if self.bot else core.UsageStore(
                core.APP_DIR / "RoleWeaver_Data" / "usage.sqlite3"
            )
            session = execution.session if execution else ""
            report = store.report(self.usage_window_var.get(), session=session)
            cost = (
                f"${report['estimated_cost']:.6f} estimated"
                if report["priced_requests"]
                else "cost unknown"
            )
    ...
    def _draw_usage_chart(self, groups, metric):
        ...
        if not items:
            canvas.create_text(12, 15, text="No usage in this window.", fill="#b5bac1", anchor="nw")
            return
        keys = {
            "Requests": "requests",
        ...
        key = keys.get(metric, "tokens")
        maximum = max(float(values[key]) for _, values in items) or 1.0
        bar_width = max(34, (width - 30) // len(items) - 12)
        for index, (name, values) in enumerate(items):
            x1 = 18 + index * (bar_width + 12)
            value = float(values[key])
            bar_height = int(75 * value / maximum)
            ...
            if key == "estimated_cost" and not values["priced_requests"]:
                label = "unknown"
            canvas.create_text(
                x1 + bar_width / 2,
                84 - bar_height,
        ...
        if self.bot:
            try:
                ...
                self.auto_btn.configure(
                    text="AFK ON (F10)" if self.bot.afk else "AFK OFF (F10)"
                )
