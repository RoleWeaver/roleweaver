...
class NWNAIApp:
    def __init__(self, root):
        ...
        style.configure(".", font=("DejaVu Sans", 9))
        ...
        if core.is_wayland():
            self.root.title('Role Weaver: NWN & NWN2 — macOS manual paste')
            self.paste_draft_btn.configure(text='Copy Edited Draft')
            self._append_log('[MAC] Generate drafts using the buttons. Copy, open NWN chat, Command+V, then Enter. Global hotkeys and auto-send are unavailable.')
        self._poll_output()
    ...
    def _build_ui(self):
        ...
        self.test_btn = ttk.Button(
            row3, text="Clipboard Test" if core.is_wayland() else "Keyboard Test", command=self.keyboard_test, state="disabled"
        )
        ...
        self.continuous_translation_var = tk.BooleanVar(value=bool(self.settings.get("translation_continuous", False)))
        ttk.Checkbutton(translation_buttons, text="Continuous Translation", variable=self.continuous_translation_var, command=self._toggle_continuous_translation).pack(side="left")
        self.translate_chat_btn = ttk.Button(translation_buttons, text="Translate Last 3", command=self.translate_recent_chat, state="disabled")
        ...
        translation_scroll = ttk.Scrollbar(translation_body, orient="vertical", command=self.translation_text.yview)
        ...
        ttk.Label(translation, textvariable=self.translation_status_var).pack(fill="x", pady=(5, 0))
        ...
        self.update_panel = UpdatePanel(
            update_frame, self.root, platform="darwin", prepare_install=lambda: False,
        ...
        self.memory_overview_text = tk.Text(
            ...
            bg="#1e1f22", fg="#dbdee1", insertbackground="#dbdee1",
            selectbackground="#5865f2", selectforeground="#ffffff",
            relief="flat", bd=0, padx=8, pady=7, font=("DejaVu Sans", 9),
        )
        ...
        for kind, title, cols, headings in specs:
            ...
            ttk.Label(head,text=title,font=("DejaVu Sans",9,"bold")).pack(side="left")
        ...
        self.correction_text=tk.Text(corr_frame,wrap="word",state="disabled",height=7,bg="#1e1f22",fg="#dbdee1",relief="flat",bd=0,padx=8,pady=7,font=("DejaVu Sans",9))
        ...
        ttk.Label(dev_frame,text="Pending proposals",font=("DejaVu Sans",9,"bold")).pack(anchor="w")
        ...
        ttk.Label(dev_frame,text="Approved development",font=("DejaVu Sans",9,"bold")).pack(anchor="w")
        ...
        ttk.Label(cast_header, text="NPC Cast Manager", font=("DejaVu Sans", 10, "bold")).pack(side="left")
        ...
        ttk.Label(cast_right, text="Selected NPC", font=("DejaVu Sans", 9, "bold")).pack(anchor="w")
        ...
        ttk.Label(cast_right, text="Shared Campaign Memory", font=("DejaVu Sans", 9, "bold")).pack(anchor="w")
        ttk.Label(
            cast_right,
        ...
        ttk.Label(cast_right, text="DM-only Notes", font=("DejaVu Sans", 9, "bold")).pack(anchor="w", pady=(7, 3))
    ...
    def _build_language_settings_tab(self, frame):
        ttk.Label(frame, text="Incoming game chat is translated into your language. Replies are drafted in your language first, then translated into the game language for final editing.", wraplength=760).pack(anchor="w", pady=(0, 10))
        languages = core.language_choices()
        form = ttk.LabelFrame(frame, text="Languages", padding=8); form.pack(fill="x")
        ...
        terms = ttk.LabelFrame(frame, text="Protected names and terms (one per line)", padding=8); terms.pack(fill="both", expand=True, pady=(10, 0))
        self.translation_terms_text = tk.Text(terms, height=8, wrap="word"); self.translation_terms_text.pack(fill="both", expand=True)
    ...
    def _save_language_settings(self):
        self.settings.update(user_language=self.user_language_var.get() or "English", game_language=self.game_language_var.get() or "English", translation_auto_detect=bool(self.translation_auto_detect_var.get()), translation_continuous=bool(self.continuous_translation_var.get()), translation_protected_terms=self.translation_terms_text.get("1.0", "end-1c").strip())
        if self.bot:
            self.bot.settings.update({key: self.settings[key] for key in ("user_language", "game_language", "translation_auto_detect", "translation_continuous", "translation_protected_terms")})
        core.save_settings(self.settings); self.translation_status_var.set("Language settings saved.")
    ...
    def _toggle_continuous_translation(self):
        ...
        if self.bot:
            ...
            if self.settings["translation_continuous"]: self.bot.action_queue.put(("translate_chat", "new"))
        core.save_settings(self.settings)

    def translate_recent_chat(self):
        if self.bot:
            self._save_language_settings(); self.bot.action_queue.put(("translate_chat", "recent"))

    def translate_current_draft(self):
        if not self.bot: return
        text = self.ai_draft_text.get("1.0", "end").strip()
        if not text:
            self._append_log("[TRANSLATION] The user-language draft is empty."); return
        self._save_language_settings(); self.bot.action_queue.put(("translate_draft", text))
    ...
    def _build_guardrails_usage_tab(self, frame):
        ...
        metric_combo = ttk.Combobox(toolbar, textvariable=self.usage_metric_var, values=["Requests", "API calls", "Tokens", "Estimated cost"], state="readonly", width=15)
    ...
    def _build_guardrail_policy_page(self, frame):
        self.guardrail_purpose_labels = {
            "Default": "", "Replies": "reply", "Candidates": "candidates",
            "Summaries": "summary", "AFK": "afk",
            "Incoming translations": "translation_incoming",
            "Outgoing translations": "translation_outgoing",
            "Connection tests": "connection_test",
        }
        selector = ttk.Frame(frame); selector.pack(fill="x", pady=(0, 6))
        ttk.Label(selector, text="Policy for:").pack(side="left")
        self.guardrail_policy_purpose_var = tk.StringVar(value="Default")
        purpose_combo = ttk.Combobox(selector, textvariable=self.guardrail_policy_purpose_var, values=list(self.guardrail_purpose_labels), state="readonly", width=18)
        purpose_combo.pack(side="left", padx=6)
        purpose_combo.bind("<<ComboboxSelected>>", self._load_guardrail_policy)
        ttk.Label(selector, text="Purpose settings inherit Default until explicitly changed.", foreground="#949ba4").pack(side="left", padx=10)
        rules = ttk.LabelFrame(frame, text="Categories and actions", padding=7); rules.pack(fill="x")
        self.guardrail_policy_vars = {}; self.guardrail_policy_combos = {}
        for index, (category, label) in enumerate(core.CATEGORY_LABELS.items()):
            column = 0 if index < 5 else 2; row = index if index < 5 else index - 5
            ttk.Label(rules, text=label + ":").grid(row=row, column=column, sticky="w", padx=(0, 8), pady=2)
            variable = tk.StringVar(); self.guardrail_policy_vars[category] = variable
            combo = ttk.Combobox(rules, textvariable=variable, values=["Off", "Warn", "Block", "Replace"], state="readonly", width=11)
            combo.grid(row=row, column=column + 1, sticky="w", padx=(0, 22), pady=2); self.guardrail_policy_combos[category] = combo
        behavior = ttk.LabelFrame(frame, text="Replacement and retry", padding=7); behavior.pack(fill="x", pady=(7, 0))
        ...
        ttk.Label(custom, text="Regular expressions (one per line):").grid(row=0, column=1, sticky="nw", padx=(12, 0))
        self.guardrail_terms_text = tk.Text(custom, height=3, wrap="word"); self.guardrail_regex_text = tk.Text(custom, height=3, wrap="none")
    ...
    def _load_guardrail_policy(self, _event=None):
        ...
        for category, variable in self.guardrail_policy_vars.items():
            values = ["Off", "Warn", "Block", "Replace"] + (["Inherit"] if purpose else []); self.guardrail_policy_combos[category].configure(values=values)
            variable.set((defaults[category] if not purpose else overrides.get(category, "inherit")).title())

    def _save_guardrail_policy(self):
        try:
            expression_text = self.guardrail_regex_text.get("1.0", "end-1c").strip(); errors = core.validate_custom_patterns(expression_text)
            if errors: raise ValueError("\n".join(errors))
            purpose = self.guardrail_purpose_labels[self.guardrail_policy_purpose_var.get()]
            if purpose:
                policies = dict(self.settings.get("guardrail_purpose_policies", {})); overrides = dict(policies.get(purpose, {}))
                for category, variable in self.guardrail_policy_vars.items():
                    action = variable.get().lower()
                    if action == "inherit": overrides.pop(category, None)
                    else: overrides[category] = action
                if overrides: policies[purpose] = overrides
                else: policies.pop(purpose, None)
                self.settings["guardrail_purpose_policies"] = policies
            else:
                defaults = dict(core.DEFAULT_ACTIONS)
                for category, variable in self.guardrail_policy_vars.items():
                    action = variable.get().lower(); defaults[category] = action if action in core.ACTION_VALUES else core.DEFAULT_ACTIONS[category]
    ...
    def _build_guardrail_events_page(self, frame):
        ...
        ttk.Label(toolbar, text="Policy events contain metadata only—never prompts or replies.").pack(side="left")
        ttk.Button(toolbar, text="Refresh", command=self._refresh_guardrail_events).pack(side="right")
        ...
        for column in columns:
            self.guardrail_events_tree.heading(column, text=column.title()); self.guardrail_events_tree.column(column, width=widths[column], anchor="w")
        event_scroll = ttk.Scrollbar(frame, orient="vertical", command=self.guardrail_events_tree.yview); self.guardrail_events_tree.configure(yscrollcommand=event_scroll.set)
        self.guardrail_events_tree.pack(side="left", fill="both", expand=True); event_scroll.pack(side="right", fill="y")

    def _refresh_guardrail_events(self):
        if not hasattr(self, "guardrail_events_tree"): return
        tree = self.guardrail_events_tree; tree.delete(*tree.get_children()); execution = self.bot.ai_execution if self.bot else None
        store = self.bot.usage_store if self.bot else core.UsageStore(core.APP_DIR / "RoleWeaver_Data" / "usage.sqlite3")
        rows = store.guardrail_events(self.usage_window_var.get(), session=execution.session if execution else "")
        for row in rows:
            timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(row["created"]))
            tree.insert("", "end", values=(timestamp, row["purpose"], row["direction"], core.CATEGORY_LABELS.get(row["category"], row["category"]), row["action"], row["backend"]))
    ...
    def _refresh_guardrails_usage(self):
        try:
            ...
            backend = execution.guardrails if execution else core.GuardrailsAIBackend(self.settings.get("guardrail_input_max_characters", 50000), self.settings.get("guardrail_output_max_characters", 10000), settings=self.settings)
            ...
            self.usage_summary_var.set(f"{report['requests']} requests • {report['provider_calls']} API calls • {report['total_tokens']:,} reported tokens • {report['unknown_token_requests']} without token data • {report['blocked']} blocked • {cost}")
    ...
    def _draw_usage_chart(self, groups, metric):
        ...
        keys = {"Requests": "requests", "API calls": "provider_calls", "Tokens": "tokens", "Estimated cost": "estimated_cost"}
    ...
    def _open_character_editor(self, edit_selected=False):
        ...
        for heading, height in sections:
            ttk.Label(form, text=heading + ":", font=("DejaVu Sans", 9, "bold")).pack(fill="x", pady=(8, 3))
    ...
    def _briefing_window(self, title, text):
        ...
        body = tk.Text(win, wrap="word", bg="#1e1f22", fg="#dbdee1", insertbackground="#dbdee1",
                       ...
                       padx=12, pady=10, font=("DejaVu Sans", 10))
    ...
    def _open_campaign_manager(self):
        ...
        ttk.Label(header, text="Campaign Manager", font=("DejaVu Sans", 12, "bold")).pack(side="left")
        ...
        # Overview
        ttk.Label(overview, text="Campaign Identity", font=("DejaVu Sans", 10, "bold")).pack(anchor="w")
    ...
    def _update_provider_ui(self):
        ...
        if provider == "LM Studio":
            ...
            if provider == "OpenAI":
                self.provider_hint_var.set(f"Uses OpenAI. Enter a key here or set {env_name} in your environment.")
            else:
                self.provider_hint_var.set(
                    f"Uses Google Gemini. Enter a key here or set {env_name} in your environment. "
    ...
    def test_ai_connection(self):
        try:
            ...
            test_service = core.AIExecutionService(client, self.settings, core.UsageStore(core.APP_DIR / "RoleWeaver_Data" / "usage.sqlite3"))
            test_service.generate(core.AIRequest(instructions="Return only the requested connection-test response.", prompt="Reply with exactly: OK", purpose=core.AIRequestPurpose.CONNECTION_TEST))
    ...
    def _confirm_character_mismatch(self, detected, loaded, matching_profile=""):
        ...
        ttk.Label(frame,text="Character Profile Mismatch",font=("DejaVu Sans",11,"bold")).pack(anchor="w")
    ...
    def start_bot(self):
        ...
        if core.AI_PROVIDERS[provider]["requires_key"] and not api_key:
            ...
            messagebox.showerror(
                ...
                f"Enter an API key or set {env_name} in your environment before starting.",
            )
            return
