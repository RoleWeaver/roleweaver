...
SERVER_PROFILES = {
    "CUSTOM": {
        "display_name": "Custom / Other NWN Server",
        "default_log_path": DEFAULT_NWN_LOG_PATH,
    },
    "TDN": {
        "display_name": "The Dragon's Neck (TDN)",
        "default_log_path": DEFAULT_NWN_LOG_PATH,
    },
    "Arelith": {
        "display_name": "Arelith",
        "default_log_path": DEFAULT_NWN_LOG_PATH,
    },
}
...
def parse_chat_line(line, character_name, server_profile="TDN"):
    raw = line.strip()
    ...
    if message.startswith("[") and message.endswith("]"):
        return None
    if server_profile == "Arelith" and speaker.casefold() == "public message board":
        return None
    return {
...
def load_relevant_lore(context_text, max_files=3, max_characters=5000):
    """Return a few locally stored lore references relevant to the current chat."""
    try:
        LORE_DIR.mkdir(parents=True, exist_ok=True)
    except Exception:
        return []

    files = [p for p in LORE_DIR.glob("*.txt") if p.is_file()]
    if not files:
        return []
    ...
    for path in files:
        ...
        # Files prefixed with always_ are deliberately always included.
...
"""
    ...
    def paste_existing_draft(self, text):
        ...
        print("[DRAFT] Focus NWN now. Pasting edited draft in 2 seconds...")
        time.sleep(2)
        if send_chat_to_nwn(text, self.settings, leave_unsent=True):
