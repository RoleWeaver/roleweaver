...
DEFAULT_SETTINGS = default_settings(DEFAULT_NWN_LOG_PATH, keyboard_method="scancode")
...
class NWNAIBot(AFKMixin):
    def __init__(self, settings, character_prompt, client):
        ...
        # Translation results are polled by the GUI just like AI drafts. Source
        # chat remains in the ordinary context; only display-ready translations
        # are retained here for this session.
    ...
    def translate_recent_chat(self, *, only_new=False, limit=3):
        ...
        source = (
            "auto"
            if self.settings.get("translation_auto_detect", True)
            else self.settings.get("game_language", "English")
        )
        ...
        try:
            ...
            for message in result.messages:
                item = {
                    ...
                    "language": message.detected_source_language.name
                    if message.detected_source_language
                    else "",
                }
                if message.id in known:
                    self.translated_chat = [
                        item if existing["id"] == message.id else existing
                        for existing in self.translated_chat
                    ]
                else:
                    self.translated_chat.append(item)
