...
def _safe_name(name):
    ...
    if (not parts or name != "/".join(parts) or "\\" in name
            or any(p in {".", ".."} or p.endswith((".", " "))
                   ...
                   for p in parts)
            or not ((len(parts) == 1 and name in FILES)
                    or (len(parts) > 1 and parts[0] in DIRECTORIES))):
...
def create_backup(root, destination, validate_json=False):
    ...
    if any(destination.is_relative_to(root / d) for d in DIRECTORIES) or destination in [root / f for f in FILES]:
        raise ValueError("Save backups outside the data being backed up")
    destination.parent.mkdir(parents=True, exist_ok=True)
    manifest = {"format": "roleweaver-backup", "version": 1,
    ...
    with tempfile.TemporaryDirectory(dir=destination.parent) as temp:
        ...
        with zipfile.ZipFile(archive, "w", zipfile.ZIP_DEFLATED) as z:
            for entry in sorted(FILES | DIRECTORIES):
                base = _target(root, entry + "/placeholder").parent if entry in DIRECTORIES else _target(root, entry)
                if not base.exists():
                    continue
...
def restore_backup(root, archive, validate_only=False, preserve_recovery_work=False):
    ...
    with tempfile.TemporaryDirectory(dir=root) as temp:
        ...
        with zipfile.ZipFile(archive) as z:
            ...
            if sum(i.file_size for i in infos) > MAX_BYTES or z.getinfo("manifest.json").file_size > 4 * 1024 * 1024:
                raise ValueError("Backup exceeds size limits")
            manifest = json.loads(z.read("manifest.json"))
            if not isinstance(manifest, dict) or manifest.get("format") != "roleweaver-backup" or manifest.get("version") != 1:
        ...
        try:
            for name in files:
                dest = _target(root, name)
                if preserve_recovery_work and (name.startswith("RoleWeaver_Data/Recovery_Drafts/") or name.endswith("/pending_summaries.json")) and dest.exists():
                    try:
            ...
            try:
                ...
                raise RuntimeError(f"Restore and rollback failed. Recovery backup: {recovery}") from rollback_error
            raise RuntimeError(f"Restore failed; original files restored. Recovery backup: {recovery}") from error
        return recovery
