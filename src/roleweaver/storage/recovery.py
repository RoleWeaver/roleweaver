...
class CrashProtection:
    ...
    def acquire(self):
        ...
        try:
            ...
            self.lock_file = None
            raise RuntimeError(
                "Another Role Weaver client is using this installation. Close it before continuing."
            ) from exc
        self.unclean = self.marker.exists()
        try:
            storage.atomic_write_text(
                self.marker,
                json.dumps({"pid": os.getpid(), "started": datetime.now(timezone.utc).isoformat()}),
            )
        except Exception:
            self.release()
            raise
    ...
    def start(self, report):
        ...
        self.worker = threading.Thread(
            target=self._run, name="RoleWeaver automatic backup", daemon=True
        )
        self.worker.start()
    ...
    def _try_snapshot(self):
        try:
            ...
            self.report(
                f"[BACKUP ERROR] Automatic backup failed: {exc}. Previous backups were retained."
            )
            return False
        ...
        if guard.unclean or problem:
            ...
            if candidate:
                choice = messagebox.askyesnocancel(
                    ...
                    "No: keep current data. Cancel: exit. Changes since the backup are not "
                    "recovered.",
                    parent=root,
                )
                if choice is None:
                    ...
                    return None
                if choice:
                    recovery = backups.restore_backup(
                        guard.root, candidate, preserve_recovery_work=True
                    )
                    backups.validate_saved_data(guard.root)
                    messagebox.showinfo(
                        "Recovery complete",
                        f"Saved data was restored. Previous files are preserved in:\n{recovery}",
                        parent=root,
                    )
                elif problem:
                    raise RuntimeError(
                        "Startup stopped to preserve damaged data. Restore a valid backup or "
                        "repair the reported file before relaunching."
                    )
            elif problem:
                raise RuntimeError(
                    reason
                    + "\n\nNo valid automatic backup is available. Existing data has been "
                    "preserved. Restore a manual backup or repair the reported file before "
                    "relaunching."
                )
            else:
                if not messagebox.askokcancel(
                    "Crash recovery",
                    reason
                    + "\n\nNo verified automatic backup is available. Continue with the saved "
                    "files?",
                    parent=root,
                ):
                    guard.release()
