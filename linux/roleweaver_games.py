...
GAME_FIELDS = ("server_profile", "log_path", "server_log_paths", "discovered_servers",
               "parser_profile", "window_title_contains")
...
def game_directories(game, home=None, environ=None, platform=None):
    ...
    if not linux:
        ...
        if not nwn2:
            return [Path(env.get("ProgramFiles(x86)", "C:/Program Files (x86)")) / "Neverwinter Nights/logs",
                    Path("C:/NeverwinterNights/NWN/logs"), Path("C:/GOG Games/Neverwinter Nights Diamond/logs")]
    ...
    if game == "nwn_ee":
        ...
        return roots
    roots = [] if nwn2 else [home / "nwn/logs", home / "Neverwinter Nights/logs",
    ...
    for prefix in dict.fromkeys(prefixes):
        if not nwn2:
            roots.extend([prefix / "drive_c/Program Files (x86)/Neverwinter Nights/logs",
                          prefix / "drive_c/GOG Games/Neverwinter Nights Diamond/logs"])
            continue
        users = prefix / "drive_c/users"
        names = list(users.iterdir()) if users.is_dir() else [users / "steamuser", users / home.name]
        for user in names:
...
def discover_game_logs(settings, max_files=20):
    explicit = [settings.get("log_path", "")] + list((settings.get("server_log_paths") or {}).values())
    candidates = [Path(p).expanduser() for p in explicit if p]
    for directory in game_directories(settings.get("game_version", "nwn_ee")):
        patterns = ("nwn2client64Log*.txt", "nwn2client64log*.txt", "nwclientLog*.txt", "nwclientlog*.txt", "*.log", "*/*.txt", "*/*.log", "*/*/*.txt", "*/*/*.log")
    ...
    for path in candidates:
        try:
            ...
            continue
    return [p for _, p in sorted(found.values(), key=lambda item: item[0], reverse=True)[:max_files]]
...
def switch_game(settings, game):
    ...
    fresh = dict(server_profile="AUTO", log_path=path, server_log_paths={"AUTO": path},
                 discovered_servers={}, parser_profile="adaptive",
                 window_title_contains="Neverwinter Nights 2" if game.startswith("nwn2") else "Neverwinter Nights")
...
NWN2_PATTERNS = [
    re.compile(r"^\[(?P<channel>" + CHANNEL + r")\]\s*(?P<speaker>[^:]{1,120}):\s*(?P<message>.+)$", re.I),
    re.compile(r"^(?P<speaker>[^:]{1,120}):\s*\[(?P<channel>" + CHANNEL + r")\]\s*(?P<message>.+)$", re.I),
    re.compile(r"^(?P<speaker>[^:\[]{1,120})\s*\[(?P<channel>" + CHANNEL + r")\]\s*:\s*(?P<message>.+)$", re.I),
]
