...
class DraftRecovery:
    ...
    def show(self):
        ...
        ttk.Label(win, text="Select an edit to review and copy back into its editor. Nothing is sent to the game. Saved edits remain here until discarded.", wraplength=800).pack(padx=10, pady=10)
        listing = tk.Listbox(win, height=8, exportselection=False)
        listing.pack(fill="x", padx=10)
        detail = tk.Text(win, wrap="word", state="disabled", exportselection=False)
