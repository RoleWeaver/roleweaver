...
class ClientFixTests(unittest.TestCase):
    ...
    def test_translation_workflow_filters_self_chat_and_publishes_editable_drafts(self):
        ...
        incoming = SimpleNamespace(
            id="2", speaker="Alice", channel="Talk", translated_text="Hello",
            detected_source_language=SimpleNamespace(name="German"),
        )
        outgoing = SimpleNamespace(translated_text="Guten Abend")
        bot.translation_service = Mock()
        bot.translation_service.translate.side_effect = [
            SimpleNamespace(messages=(incoming,)),
            SimpleNamespace(message=Mock(return_value=outgoing)),
        ]

        bot.translate_recent_chat()
        bot.translate_draft("Good evening")
