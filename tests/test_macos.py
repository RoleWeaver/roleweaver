...
class MacPlatformTests(unittest.TestCase):
    def test_packaged_guardrails_smoke_reports_failure(self):
        with tempfile.TemporaryDirectory() as temp:
            result = Path(temp) / "guardrails.txt"
            with patch("roleweaver.guardrails.GuardrailsAIBackend") as backend:
                backend.return_value.error = "missing grammar"
                self.assertFalse(macos_start.guardrails_smoke(result))
            self.assertIn("missing grammar", result.read_text(encoding="utf-8"))

    def test_packaged_guardrails_smoke_validates(self):
        with tempfile.TemporaryDirectory() as temp:
            result = Path(temp) / "guardrails.txt"
            with patch("roleweaver.guardrails.GuardrailsAIBackend") as backend:
                backend.return_value.error = ""
                self.assertTrue(macos_start.guardrails_smoke(result))
                backend.return_value.validate_output.assert_called_once()
            self.assertEqual(result.read_text(encoding="utf-8"), "active")
    ...
    def test_architecture_specific_download(self):
        with patch("roleweaver.updates.system_platform.machine", return_value="arm64"):
            self.assertEqual(asset_name("1.3.2", "darwin"), "RoleWeaver-v1.3.2-macOS-arm64.zip")
